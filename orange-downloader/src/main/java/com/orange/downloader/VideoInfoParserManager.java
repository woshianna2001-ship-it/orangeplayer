...
import com.orange.downloader.m3u8.M3U8Utils;
...
public class VideoInfoParserManager {
    ...
    private void doParseVideoInfoTask(VideoTaskItem taskItem, IVideoInfoListener listener, Map<String, String> headers) {
        try {
            ...
            if (finalUrl.contains(Video.TypeInfo.M3U8) || VideoDownloadUtils.isM3U8Mimetype(contentType)) {
                ...
                // 设置 MimeType - 根据 contentType 或 URL 扩展名检测
                String mimeType = detectMimeType(finalUrl, contentType);
                taskItem.setMimeType(mimeType);
                
                // 设置视频类型
                int videoType = detectVideoType(mimeType);
                taskItem.setVideoType(videoType);
    ...
    private void parseNetworkM3U8Info(VideoTaskItem taskItem, Map<String, String> headers, IVideoInfoListener listener) {
        try {
            ...
            if (m3u8.hasEndList()) {
                ...
                if (!dir.exists()) {
                    dir.mkdirs();  // 使用 mkdirs() 创建所有必需的父目录
                }
    ...
    /**
     * 检测视频的 MimeType
     * 优先使用 contentType，如果为空或通用类型则从 URL 扩展名检测
     */
    private String detectMimeType(String url, String contentType) {
        // 如果 contentType 有效且不是通用类型，直接使用
        ...
        if (lowerUrl.endsWith(Video.SUFFIX.SUFFIX_MP4) || lowerUrl.contains(".mp4?")) {
            return Video.Mime.MIME_TYPE_MP4;
        } else if (lowerUrl.endsWith(Video.SUFFIX.SUFFIX_WEBM) || lowerUrl.contains(".webm?")) {
            return Video.Mime.MIME_TYPE_WEBM;
        } else if (lowerUrl.endsWith(Video.SUFFIX.SUFFIX_MOV) || lowerUrl.contains(".mov?")) {
            return Video.Mime.MIME_TYPE_QUICKTIME;
        } else if (lowerUrl.endsWith(Video.SUFFIX.SUFFIX_3GP) || lowerUrl.contains(".3gp?")) {
            return Video.Mime.MIME_TYPE_3GP;
        } else if (lowerUrl.endsWith(Video.SUFFIX.SUFFIX_MKV) || lowerUrl.contains(".mkv?")) {
            return Video.Mime.MIME_TYPE_MKV;
        } else if (lowerUrl.contains("flv") || lowerUrl.contains(".flv?")) {
            return "video/x-flv";
        }
     ...
     * 根据 MimeType 检测视频类型
     */
    private int detectVideoType(String mimeType) {
        if (TextUtils.isEmpty(mimeType)) {
            return Video.Type.DEFAULT;
        }
        
        if (mimeType.equals(Video.Mime.MIME_TYPE_MP4)) {
            return Video.Type.MP4_TYPE;
        } else if (mimeType.equals(Video.Mime.MIME_TYPE_WEBM)) {
            return Video.Type.WEBM_TYPE;
        } else if (mimeType.equals(Video.Mime.MIME_TYPE_QUICKTIME)) {
            return Video.Type.QUICKTIME_TYPE;
        } else if (mimeType.equals(Video.Mime.MIME_TYPE_3GP)) {
            return Video.Type.GP3_TYPE;
        } else if (mimeType.equals(Video.Mime.MIME_TYPE_MKV)) {
